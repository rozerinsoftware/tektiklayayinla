...
function AlanSatiri({ alan, deger, onSec, hata }) {
  ...
  return (
    <TouchableOpacity style={styles.secimSatir} onPress={() => onSec(alan.key, null, alan)} activeOpacity={0.7}>
      <View style={styles.secimSol}>
        <Text style={styles.secimLabel}>{alan.label}{alan.zorunlu ? ' *' : ''}</Text>
...
export default function EmlakTemelBilgiScreen({ navigation, route }) {
  ...
  const devamEt = () => {
    ...
    if (Object.keys(yeniHatalar).length) {
      ...
      Alert.alert('Eksik alan', 'Lütfen zorunlu emlak alanlarını doldurun.');
      return;
    }
    const detay = emlakDetayNormalize(alanDegerleri, profil);
    navigation.navigate('IlanKonumBaslangic', {
      taslakIlan: {
        baslik: baslik.trim(),
        aciklama: aciklama.trim(),
        fiyat: String(Math.round(fiyatSayi)),
        kategori: kokIdToMetaKey('emlak'),
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.tipKart}>
          ...
          <View style={styles.tipMetin}>
            <Text style={styles.tipBaslik}>{profil.emlakTipi}</Text>
            <Text style={styles.tipAlt}>{profil.ilanTuru || secilenKategori?.kategoriEtiket}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  tipMetin: { flex: 1 },
  tipBaslik: { fontSize: 16, fontWeight: '700', color: colors.link },

...
function mesajlarSonrasiGit(navigation) {
  navigation.reset({
    index: 0,
    routes: [
      {
        ...
        state: {
          routes: [
            ...
            { name: 'Ara' },
            {
              name: 'Profilim',
              state: {
                routes: [{ name: 'Profil' }, { name: 'Mesajlar' }],
                index: 1,
              },
            },
          ],
...
export default function GirisMesajScreen({ navigation, route }) {
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      >
        <View style={styles.ustBar}>
          <Text style={styles.ustBaslik}>İlan mesajlarınızı görüntülemek için giriş yapın</Text>
        ...
        >
          ...
          <View style={styles.yasalSatir}>
            <TouchableOpacity onPress={() => navigation.navigate('KurumsalBilgi', { baslangic: 'sozlesme' })}>
...
const styles = StyleSheet.create({
  ...
  ustBaslik: { flex: 1, fontSize: 22, fontWeight: '800', color: colors.text, lineHeight: 28 },
  kapat: { paddingTop: 2 },
  scroll: { paddingHorizontal: spacing.lg, paddingBottom: spacing.xxl },
  etiket: { fontSize: 14, fontWeight: '600', color: colors.text, marginBottom: 6, marginTop: spacing.md },
  input: {
    borderWidth: 1,
  ...
  yasalSatir: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', marginTop: spacing.xxl },
